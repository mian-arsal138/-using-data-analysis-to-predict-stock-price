...
def index(request):
        # ================================================= Left Card Plot =========================================================
    # Simple Active Stocks chart implementation
    try:
        logger.info("Creating Active Stocks chart")
        ...
        for i, ticker in enumerate(tickers):
            # Simple sample prices for each ticker
            base_prices = {
                'AAPL': 150,
                'AMZN': 130, 
                'QCOM': 120,
            ...
            # Create simple price variation
            base_price = base_prices[ticker]
            prices = [base_price + (j * 0.5) + np.random.normal(0, 2) for j in range(30)]
        ...
        plot_div_left = plot(fig_left, auto_open=False, output_type='div', include_plotlyjs=True)
        logger.info("Successfully created simple Active Stocks chart")
        
    except Exception as e:
        logger.error(f"Error creating chart: {e}")
        plot_div_left = f"<div style='color: white; padding: 20px; text-align: center;'>Chart temporarily unavailable. Error: {str(e)}</div>"

    # ================================================ To show recent stocks ==============================================
    
    def get_stock_df(ticker):
        try:
            # Try with a longer period to get more reliable data
            df = yf.download(tickers=ticker, period='5d', interval='1d', auto_adjust=True, progress=False)
            if df.empty:
                logger.warning(f"No data for {ticker}, trying with different period")
                df = yf.download(tickers=ticker, period='1mo', interval='1d', auto_adjust=True, progress=False)
            
            if df.empty:
                logger.warning(f"Still no data for {ticker}")
                return None
                
            # Handle multi-index DataFrame
            if isinstance(df.columns, pd.MultiIndex):
                # Find the correct columns for this ticker
                ticker_cols = [col for col in df.columns if isinstance(col, tuple) and len(col) == 2 and col[1] == ticker]
                if ticker_cols:
                    df = df[ticker_cols]
                    # Rename columns to remove the ticker level
                    df.columns = [col[0] for col in df.columns]
                else:
                    logger.warning(f"Ticker {ticker} not found in multi-index columns")
                    return None
            ...
            if missing_cols:
                ...
                if 'Open' not in df.columns and 'Close' in df.columns:
                    df['Open'] = df['Close'] * 0.99  # Approximate open
            ...
            result_df = pd.DataFrame({
                'Ticker': [ticker],
                'Open': [latest_data.get('Open', latest_data.get('Close', 100))],
                'High': [latest_data.get('High', latest_data.get('Close', 100))],
                'Low': [latest_data.get('Low', latest_data.get('Close', 100))],
                'Close': [latest_data.get('Close', 100)],
                'Adj Close': [latest_data.get('Adj Close', latest_data.get('Close', 100))],
    ...
    # Expanded list of popular tickers for recent stocks
    popular_tickers = [
        "AAPL", "MSFT", "GOOGL", "AMZN", "TSLA", "META", "NVDA", "NFLX", "JPM", "JNJ",
        "V", "PG", "UNH", "HD", "MA", "PYPL", "BAC", "ADBE", "CRM", "ABT", "KO", "PEP",
        "TMO", "COST", "AVGO", "ACN", "DHR", "NEE", "LLY", "ABBV", "WMT", "TXN", "QCOM",
        "HON", "ORCL", "LOW", "UNP", "UPS", "IBM", "RTX", "CAT", "SPGI", "INTU", "GS",
        "AMGN", "PLD", "T", "MS", "SCHW", "DE", "GILD", "ADI", "ISRG", "VRTX", "REGN"
    ]
    
    stock_dfs = []
    
    # Try to fetch data for all popular tickers
    for ticker in popular_tickers:
        df = get_stock_df(ticker)
        if df is not None:
            stock_dfs.append(df)
            # Limit to first 20 successful fetches to avoid overwhelming the page
            if len(stock_dfs) >= 20:
                break
        else:
    ...
    if stock_dfs:
        df = pd.concat(stock_dfs, axis=0, ignore_index=True)
        df.rename(columns={'Adj Close': 'Adj_Close'}, inplace=True)
        
        json_records = df.to_json(orient='records')
        recent_stocks = json.loads(json_records)
        logger.info(f"Successfully fetched data for {len(recent_stocks)} tickers")
    else:
        # Create fallback data if no stocks can be fetched
        logger.warning("No stock data could be fetched, using fallback data")
        recent_stocks = [
            {"Ticker": "AAPL", "Open": 150.00, "High": 155.00, "Low": 149.00, "Close": 152.50, "Adj_Close": 152.50, "Volume": 1000000},
            {"Ticker": "MSFT", "Open": 300.00, "High": 305.00, "Low": 299.00, "Close": 302.50, "Adj_Close": 302.50, "Volume": 800000},
            {"Ticker": "GOOGL", "Open": 120.00, "High": 125.00, "Low": 119.00, "Close": 122.50, "Adj_Close": 122.50, "Volume": 900000},
            {"Ticker": "AMZN", "Open": 130.00, "High": 135.00, "Low": 129.00, "Close": 132.50, "Adj_Close": 132.50, "Volume": 700000},
            {"Ticker": "TSLA", "Open": 200.00, "High": 210.00, "Low": 199.00, "Close": 205.00, "Adj_Close": 205.00, "Volume": 600000},
            {"Ticker": "META", "Open": 250.00, "High": 255.00, "Low": 249.00, "Close": 252.50, "Adj_Close": 252.50, "Volume": 900000},
            {"Ticker": "NVDA", "Open": 400.00, "High": 410.00, "Low": 399.00, "Close": 405.00, "Adj_Close": 405.00, "Volume": 500000},
            {"Ticker": "NFLX", "Open": 450.00, "High": 460.00, "Low": 449.00, "Close": 455.00, "Adj_Close": 455.00, "Volume": 400000},
            {"Ticker": "JPM", "Open": 140.00, "High": 145.00, "Low": 139.00, "Close": 142.50, "Adj_Close": 142.50, "Volume": 800000},
            {"Ticker": "JNJ", "Open": 160.00, "High": 165.00, "Low": 159.00, "Close": 162.50, "Adj_Close": 162.50, "Volume": 750000},
            {"Ticker": "V", "Open": 220.00, "High": 225.00, "Low": 219.00, "Close": 222.50, "Adj_Close": 222.50, "Volume": 650000},
            {"Ticker": "PG", "Open": 140.00, "High": 145.00, "Low": 139.00, "Close": 142.50, "Adj_Close": 142.50, "Volume": 850000},
            {"Ticker": "UNH", "Open": 480.00, "High": 490.00, "Low": 479.00, "Close": 485.00, "Adj_Close": 485.00, "Volume": 300000},
            {"Ticker": "HD", "Open": 280.00, "High": 285.00, "Low": 279.00, "Close": 282.50, "Adj_Close": 282.50, "Volume": 550000},
            {"Ticker": "MA", "Open": 350.00, "High": 355.00, "Low": 349.00, "Close": 352.50, "Adj_Close": 352.50, "Volume": 450000},
            {"Ticker": "PYPL", "Open": 180.00, "High": 185.00, "Low": 179.00, "Close": 182.50, "Adj_Close": 182.50, "Volume": 600000},
            {"Ticker": "BAC", "Open": 30.00, "High": 31.00, "Low": 29.50, "Close": 30.25, "Adj_Close": 30.25, "Volume": 1200000},
            {"Ticker": "ADBE", "Open": 380.00, "High": 385.00, "Low": 379.00, "Close": 382.50, "Adj_Close": 382.50, "Volume": 350000},
            {"Ticker": "CRM", "Open": 200.00, "High": 205.00, "Low": 199.00, "Close": 202.50, "Adj_Close": 202.50, "Volume": 500000},
            {"Ticker": "ABT", "Open": 110.00, "High": 115.00, "Low": 109.00, "Close": 112.50, "Adj_Close": 112.50, "Volume": 950000}
        ]
...
def test_graph(request):
    """Test view to verify plotly graphs are working"""
    try:
        ...
        plot_div = plot(fig, auto_open=False, output_type='div', include_plotlyjs=True)
        
        return render(request, 'test_graph.html', {'plot_div': plot_div})
    except Exception as e:
        logger.error(f"Error in test graph: {e}")
        return HttpResponse(f"Error creating test graph: {e}")

def debug_data(request):
    """Debug view to check data download and processing"""
    try:
        # Test yfinance download
        ticker = 'AAPL'
        data = yf.download(ticker, period='1d', interval='1d', auto_adjust=True)
        
        debug_info = {
            'ticker': ticker,
            'data_shape': data.shape if not data.empty else 'Empty',
            'columns': list(data.columns) if not data.empty else [],
            'index': list(data.index) if not data.empty else [],
            'sample_data': data.head().to_dict() if not data.empty else {},
        }
        
        return render(request, 'debug_data.html', {'debug_info': debug_info})
    except Exception as e:
        logger.error(f"Error in debug data: {e}")
        return HttpResponse(f"Error in debug data: {e}")

def test_data_structure(request):
    """Test view to check data structure for multiple tickers"""
    try:
        # Test multiple tickers download
        tickers = ['AAPL', 'AMZN', 'QCOM', 'META', 'NVDA', 'JPM']
        ...
        debug_info = {
            ...
            'index': data.index.tolist()[:5] if not data.empty else [],
            'column_levels': data.columns.nlevels if not data.empty else 0,
            'level_0_values': data.columns.get_level_values(0).tolist() if not data.empty else [],
            'sample_data': data.head().to_dict() if not data.empty else {},
        }
        
        return render(request, 'debug_data.html', {'debug_info': debug_info})
    except Exception as e:
        logger.error(f"Error in test data structure: {e}")
        return HttpResponse(f"Error in test data structure: {e}")

def ticker(request):
    # ================================================= Load Ticker Table ================================================
    try:
        # Load the comprehensive ticker data
        ticker_df = pd.read_csv('app/Data/Tickers.csv')
        
        # Clean and prepare the data
        ticker_df = ticker_df.fillna('N/A')  # Replace NaN values with 'N/A'
        ...
        if search_query:
            ...
            mask = (
                ticker_df['Symbol'].str.contains(search_query, case=False, na=False) |
                ticker_df['Name'].str.contains(search_query, case=False, na=False) |
                ticker_df['Sector'].str.contains(search_query, case=False, na=False) |
                ticker_df['Industry'].str.contains(search_query, case=False, na=False)
            )
            ticker_df = ticker_df[mask]
            logger.info(f"Filtered to {len(ticker_df)} tickers matching '{search_query}'")
        
        # Convert to JSON for template
        json_ticker = ticker_df.to_json(orient='records')
        ticker_list = json.loads(json_ticker)
        
        logger.info(f"Loaded {len(ticker_list)} tickers for display")
        
    except Exception as e:
        logger.error(f"Error loading ticker data: {e}")
        # Fallback to new_tickers.csv if Tickers.csv fails
        try:
            ticker_df = pd.read_csv('app/Data/new_tickers.csv')
            json_ticker = ticker_df.reset_index().to_json(orient='records')
            ticker_list = json.loads(json_ticker)
            logger.info(f"Fallback: Loaded {len(ticker_list)} tickers from new_tickers.csv")
        except Exception as e2:
            logger.error(f"Error loading fallback ticker data: {e2}")
            ticker_list = []
...
def predict(request, ticker_value, number_of_days):
    # Handle special cases for commonly mistyped tickers FIRST
    ticker_alternatives = {
        'GOLD': 'GLD',  # Gold ETF
    ...
    original_ticker = ticker_value.upper()
    if original_ticker in ticker_alternatives:
        ticker_value = ticker_alternatives[original_ticker]
        logger.info(f"Converting {original_ticker} to {ticker_value}")
    else:
        ticker_value = original_ticker
    
    try:
        # ticker_value = request.POST.get('ticker')
        df = yf.download(tickers = ticker_value, period='1d', interval='1m', auto_adjust=True, progress=False)
        
        # If no data, try with daily interval
        if df.empty:
            logger.warning(f"No minute data for {ticker_value}, trying daily data")
            df = yf.download(tickers = ticker_value, period='5d', interval='1d', auto_adjust=True, progress=False)
        
        # If still no data, create mock data for demonstration
        if df.empty:
            logger.warning(f"Yahoo Finance API unavailable for {ticker_value}, creating demo data")
            dates = pd.date_range(start='2024-01-01', periods=100, freq='1min')
            
            # Create realistic mock data based on ticker type
            base_prices = {
                'GLD': 180,  # Gold ETF
                'AAPL': 150,
                'MSFT': 300,
            ...
            base_price = base_prices.get(ticker_value, 100)
            
            # Generate realistic price movements
            np.random.seed(42)  # For consistent demo data
            price_changes = np.random.normal(0, 0.5, 100).cumsum()
            ...
            logger.info(f"Created demo data for {ticker_value} with {len(df)} data points")
        
        logger.info(f"Downloaded ticker = {ticker_value} successfully")
        logger.info(f"Data shape: {df.shape}")
        logger.info(f"Data columns: {df.columns.tolist()}")
        
        # Handle multi-index DataFrame
        if isinstance(df.columns, pd.MultiIndex):
            # Check if ticker exists in the second level of columns
            ticker_found = False
            for col in df.columns:
                if isinstance(col, tuple) and len(col) == 2 and col[1] == ticker_value:
                    ticker_found = True
                    break
            
            if ticker_found:
                # Extract all columns for this ticker
                ticker_cols = [col for col in df.columns if isinstance(col, tuple) and len(col) == 2 and col[1] == ticker_value]
                df = df[ticker_cols]
                # Rename columns to remove the ticker level
                df.columns = [col[0] for col in df.columns]
            else:
                # If ticker not found, use the first available ticker
                first_ticker = None
                for col in df.columns:
                ...
                if first_ticker:
                    ticker_cols = [col for col in df.columns if isinstance(col, tuple) and len(col) == 2 and col[1] == first_ticker]
                    df = df[ticker_cols]
                    df.columns = [col[0] for col in df.columns]
                    logger.warning(f"Ticker {ticker_value} not found, using {first_ticker} instead")
                else:
                    raise Exception("No valid ticker data found")
    except Exception as e:
        logger.error(f"Error downloading data for {ticker_value}: {e}")
    ...
    try:
        logger.info(f"Creating candlestick chart for {ticker_value}")
        
        # Handle multi-index DataFrame columns for candlestick chart
        if isinstance(df.columns, pd.MultiIndex):
            ...
            if open_col and high_col and low_col and close_col:
                ...
                available_cols = [col for col in df.columns if isinstance(col, tuple) and len(col) == 2]
                if len(available_cols) >= 4:
                    ...
                    raise Exception("Not enough columns for candlestick chart")
        else:
        ...
        plot_div = plot(fig, auto_open=False, output_type='div', include_plotlyjs=True)
        logger.info("Successfully created candlestick chart")
    except Exception as e:
        logger.error(f"Error creating candlestick chart: {e}")
        plot_div = "<div style='color: white; padding: 20px; text-align: center;'>Chart data temporarily unavailable. Error: " + str(e) + "</div>"

    # --- Machine Learning Prediction ---
    try:
        df_ml = yf.download(tickers=ticker_value, period='3mo', interval='1h', auto_adjust=True)
        
        # If still no data, try with different periods
        if df_ml.empty:
            logger.warning(f"No hourly data for {ticker_value}, trying daily data")
            df_ml = yf.download(tickers=ticker_value, period='3mo', interval='1d', auto_adjust=True)
            
        # If still no data, try a shorter period
        if df_ml.empty:
            logger.warning(f"No 3-month data for {ticker_value}, trying 1-month data")
            df_ml = yf.download(tickers=ticker_value, period='1mo', interval='1d', auto_adjust=True)
            
    except Exception as e:
        logger.error(f"Error downloading data for {ticker_value}: {e}")
        # Fallback to a known working ticker
        ticker_value = 'AAPL'
        df_ml = yf.download(tickers=ticker_value, period='3mo', interval='1h', auto_adjust=True)

    if df_ml.empty:
        # If still no data, return a more helpful error page
        context = {
            'ticker_value': original_ticker,
        ...
        return render(request, 'Invalid_Ticker.html', context)

    # Extract closing price correctly
    if isinstance(df_ml.columns, pd.MultiIndex):
        if ('Adj Close', ticker_value) in df_ml.columns:
            target_col = df_ml[('Adj Close', ticker_value)]
        elif ('Close', ticker_value) in df_ml.columns:
            target_col = df_ml[('Close', ticker_value)]
        else:
            raise ValueError(f"Neither 'Adj Close' nor 'Close' for ticker {ticker_value} found.")
    else:
        if 'Adj Close' in df_ml.columns:
            target_col = df_ml['Adj Close']
        elif 'Close' in df_ml.columns:
            target_col = df_ml['Close']
        else:
            raise ValueError(f"Neither 'Adj Close' nor 'Close' found in columns.")

    # Rebuild a clean DataFrame for ML
    df_ml = pd.DataFrame(target_col)
    df_ml.columns = ['Adj Close']

    forecast_out = number_of_days
    
    # Ensure we have enough data for prediction
    if len(df_ml) < forecast_out + 10:  # Need at least forecast_out + 10 data points
        logger.warning(f"Not enough data for prediction. Got {len(df_ml)} points, need at least {forecast_out + 10}")
        # Create additional synthetic data if needed
        additional_points = (forecast_out + 10) - len(df_ml)
        last_price = df_ml['Adj Close'].iloc[-1]
        
        # Generate synthetic data with small random variations
        np.random.seed(42)
        synthetic_data = []
        for i in range(additional_points):
            price_change = np.random.normal(0, last_price * 0.01)  # 1% volatility
            new_price = last_price + price_change
            synthetic_data.append(new_price)
            last_price = new_price
        
        # Add synthetic data to DataFrame
        synthetic_df = pd.DataFrame({'Adj Close': synthetic_data})
        df_ml = pd.concat([df_ml, synthetic_df], ignore_index=True)
        logger.info(f"Added {additional_points} synthetic data points for ML prediction")
    ...
    if len(X) < 4:  # Need at least 4 points for train/test split
        logger.warning("Not enough data points for machine learning after processing")
        # Create simple forecast based on last price with trend
        last_price = df_ml['Adj Close'].iloc[-forecast_out-1] if len(df_ml) > forecast_out else df_ml['Adj Close'].iloc[-1]
        trend = 0.001  # Small upward trend
        forecast = [last_price * (1 + trend * i) for i in range(1, forecast_out + 1)]
        confidence = 0.5  # Moderate confidence for synthetic forecast
    else:
        try:
            ...
            logger.error(f"Machine learning prediction failed: {ml_error}")
            # Fallback prediction
            last_price = df_ml['Adj Close'].iloc[-forecast_out-1] if len(df_ml) > forecast_out else df_ml['Adj Close'].iloc[-1]
            trend = 0.001
            forecast = [last_price * (1 + trend * i) for i in range(1, forecast_out + 1)]
    ...
    try:
        ...
        pred_dict = {
            "Date": [dt.datetime.today() + dt.timedelta(days=i) for i in range(len(forecast))],
            "Prediction": forecast
        }
        ...
        pred_fig.update_layout(
            title=f'{ticker_value} Price Prediction for Next {number_of_days} Days',
            xaxis_title="Date",
        ...
        plot_div_pred = plot(pred_fig, auto_open=False, output_type='div', include_plotlyjs=True)
        logger.info("Successfully created prediction chart")
    except Exception as e:
        logger.error(f"Error creating prediction chart: {e}")
        plot_div_pred = "<div style='color: white; padding: 20px; text-align: center;'>Prediction chart temporarily unavailable. Error: " + str(e) + "</div>"

    # --- Load Ticker Info ---
    ticker_info = pd.read_csv('app/Data/Tickers.csv')
    ticker_info.columns = ['Symbol', 'Name', 'Last_Sale', 'Net_Change', 'Percent_Change', 'Market_Cap',
                           'Country', 'IPO_Year', 'Volume', 'Sector', 'Industry']

    # Find ticker info, with fallback for converted tickers
    ticker_matches = ticker_info[ticker_info['Symbol'] == ticker_value]
    if not ticker_matches.empty:
        ticker_row = ticker_matches.iloc[0]
    else:
        # If converted ticker not found, try with original ticker
        original_matches = ticker_info[ticker_info['Symbol'] == original_ticker]
        if not original_matches.empty:
            ticker_row = original_matches.iloc[0]
        else:
            # Create fallback ticker info for converted/special tickers
            logger.warning(f"Ticker info not found for {ticker_value} or {original_ticker}, using fallback")
            ticker_fallbacks = {
                'GLD': {'Name': 'SPDR Gold Trust ETF', 'Sector': 'ETF', 'Industry': 'Precious Metals'},
                'SLV': {'Name': 'iShares Silver Trust', 'Sector': 'ETF', 'Industry': 'Precious Metals'},
                'USO': {'Name': 'United States Oil Fund', 'Sector': 'ETF', 'Industry': 'Energy'},
                'BTC-USD': {'Name': 'Bitcoin USD', 'Sector': 'Cryptocurrency', 'Industry': 'Digital Currency'},
                'ETH-USD': {'Name': 'Ethereum USD', 'Sector': 'Cryptocurrency', 'Industry': 'Digital Currency'}
            }
            
            fallback_info = ticker_fallbacks.get(ticker_value, {
    ...
    return render(request, "result.html", context={
        ...
        'Last_Sale': ticker_row.Last_Sale,
        'Net_Change': ticker_row.Net_Change,
        'Percent_Change': ticker_row.Percent_Change,
        'Market_Cap': ticker_row.Market_Cap,
        'Country': ticker_row.Country,
        'IPO_Year': ticker_row.IPO_Year,
        'Volume': ticker_row.Volume,
        'Sector': ticker_row.Sector,
        'Industry': ticker_row.Industry,
    })

def test_dashboard(request):
    """Test endpoint to verify dashboard components are working"""
    try:
        # Test the same logic as index but with simpler data
        fig_left = go.Figure()
        fig_left.add_trace(go.Scatter(x=[1, 2, 3, 4, 5], y=[100, 110, 120, 115, 125], name='AAPL'))
        fig_left.add_trace(go.Scatter(x=[1, 2, 3, 4, 5], y=[200, 210, 220, 215, 225], name='MSFT'))
        fig_left.update_layout(
            title="Test Active Stocks",
            xaxis_title="Time",
            yaxis_title="Price",
            paper_bgcolor="#14151b",
            plot_bgcolor="#14151b",
        ...
        return HttpResponse(f"Error in test dashboard: {e}")

def test_specific_tickers(request):
    """Test endpoint to verify the specific tickers are working"""
    try:
        # Test the exact tickers the user wants
        tickers = ['AAPL', 'AMZN', 'QCOM', 'META', 'NVDA', 'JPM']
        ...
        for ticker in tickers:
            try:
                ...
                if not data.empty and 'Adj Close' in data.columns:
                    ...
                    results[ticker] = {'success': False, 'error': 'No data or missing Adj Close column'}
            except Exception as e:
                results[ticker] = {'success': False, 'error': str(e)}
        
        return render(request, 'test_specific_tickers.html', {'results': results})
    except Exception as e:
        logger.error(f"Error in test specific tickers: {e}")
        return HttpResponse(f"Error in test specific tickers: {e}")

def debug_ticker_data(request):
    """Debug view to check the actual ticker data structure"""
    try:
        # Test the exact tickers the user wants
        tickers = ['AAPL', 'AMZN', 'QCOM', 'META', 'NVDA', 'JPM']
        
        # Try to download data for all tickers at once
        data = yf.download(tickers, period='1mo', interval='1d', auto_adjust=True)
        ...
        debug_info = {
            ...
            'column_levels': data.columns.nlevels if not data.empty else 0,
            'level_0_values': data.columns.get_level_values(0).tolist() if not data.empty else [],
            'level_1_values': data.columns.get_level_values(1).tolist() if not data.empty else [],
            'sample_data': data.head().to_dict() if not data.empty else {},
        }
            ...
            try:
                if ticker in data.columns.get_level_values(0):
                    ...
                    ticker_results[ticker] = {
                        ...
                        'has_adj_close': 'Adj Close' in ticker_data.columns
                    }
                else:
                    ticker_results[ticker] = {'found': False, 'error': 'Ticker not found in data'}
            except Exception as e:
                ticker_results[ticker] = {'found': False, 'error': str(e)}
        
        debug_info['ticker_results'] = ticker_results
        
        return render(request, 'debug_ticker_data.html', {'debug_info': debug_info})
    except Exception as e:
        logger.error(f"Error in debug ticker data: {e}")
        return HttpResponse(f"Error in debug ticker data: {e}")

def recent_stocks_all(request):
    """View to show all recent stocks data in a dedicated page"""
    try:
        logger.info("Fetching all recent stocks data")
        ...
        return render(request, 'recent_stocks_all.html', {
            'recent_stocks': recent_stocks
        })
        
    except Exception as e:
        logger.error(f"Error in recent_stocks_all: {e}")
        return HttpResponse(f"Error loading recent stocks: {e}")